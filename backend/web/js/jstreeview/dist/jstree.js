...
(function (factory) {
    ...
    $.jstree.core.prototype = {
            ...
            this.element
                ...
                .on("dblclick.jstree", ".jstree-anchor", $.proxy(function (e) {
                    ...
                    if(this.settings.core.dblclick_toggle) {
                        if (this.is_parent(e.target)){
                            this.toggle_node(e.target);
                        }else{
                            var id = getid(e.target.text);
                            window.location.href = "/admin/document/update?id=" + id;
                        }
                    }
                }, this))
        ...
        create_node : function (par, node, pos, callback, is_loaded) {
            ...
            if(!node) { node = { "text" : this.get_string('Новый документ (0)') }; }
            if(typeof node === "string") { node = { "text" : node }; }
            if(node.text === undefined) { node.text = this.get_string('Новый документ (0)'); }
            var tmp, dpc, i, j;
    ...
    $.jstree.defaults.contextmenu = {
        ...
        // Шаблон контекстного меню для дерева
            ...
            return {
                "view" : {  // Просмотр документа
                    ...
                    "_disabled"			: function (data) {
                        ...
                        if (id == "1" || id == "0"){    // Нельзя просмотреть корневой и новый документы
                            return true;
                        } else{
                            return false;
                        }
                ...
                "create" : {    // Создание дочернего документа
                    "separator_before"	: false,
                    "separator_after"	: false,
                    "_disabled"			: function (data) {
                        var inst = $.jstree.reference(data.reference),
                            obj = inst.get_node(data.reference);
                        var id = getid(obj.text);
                        if (id == "0"){ // Нельзя создать дочерний документ от нового докуента
                            return true;
                        } else{
                            return false;
                        }
                    },
                    "label"				: "Создать дочерний документ",
                    "action"			: function (data) {
                        var inst = $.jstree.reference(data.reference),
                            obj = inst.get_node(data.reference);
                        var id = getid(obj.text);
                ...
                "Edit" : {  // Редактирование документа
                    ...
                    "_disabled"			: function (data) {
                        var inst = $.jstree.reference(data.reference),
                            obj = inst.get_node(data.reference);
                        var id = getid(obj.text);
                        if (id == "1" || id == "0"){    // Нельзя редактировать корневой и новый документы
                            return true;
                        }
                        else{
                            return false;
                ...
                "remove" : {    // Удаление документа с дочерними документами
                    "separator_before"	: false,
                    "icon"				: false,
                    "separator_after"	: false,
                    "_disabled"			:  function (data) {
                        var inst = $.jstree.reference(data.reference),
                            obj = inst.get_node(data.reference);
                        var id = getid(obj.text);
                        if (id == "1"){ //  Нельзя удалить корневой документ
                            return true;
                        } else{
                            return false;
                        }
                    ...
                    "action"			: function (data) {
                        ...
                        if(inst.is_selected(obj)) {
                            ...
                            if (id =="0"){
                                window.location.href = "/admin/";   // Редирект на главную страницу если находимся на новом документе и удаляем его
                            }
                            else {
                                $.ajax({
                                    url: "/admin/document/delete?id="+id,   // Удаление документа с дочерними документами
                                    success: function(data){
                                        inst.delete_node(inst.get_selected());
                                        var isparent = inst.is_parent(parent);
                                        if (!isparent){ // После удаления меняем значок родителького документа с папки на файл если не осталось больше потомков
                                            if (inst.get_icon(parent) =='/admin/css/image/icon-folder.png'){
                                                inst.set_icon(parent,'/admin/css/image/icon-file.png');
                                            } else  // После удаления меняем значок родителького документа с неопубликованной папки на неопубликованный файл если не осталось больше потомков
                                            if (inst.get_icon(parent) =='/admin/css/image/icon-folder-disable.png'){
                                                inst.set_icon(parent,'/admin/css/image/icon-file-disable.png');
                                            }
                                        }
    ...
    $.jstree.plugins.unique = function (options, parent) {
        ...
        this.create_node = function (par, node, pos, callback, is_loaded) {
            if(!node || node.text === undefined) {
                ...
                n = tmp = this.get_string('Новый документ (0)');
                dpc = [];
                for(i = 0, j = par.children.length; i < j; i++) {
